...
function createStarterDeck() {
    const cards = [];
    const types = ["shikigami", "shikigami", "spell", "awaken", "attach", "barrier"];
...
function spellEntryFromCard(card) {
    return {
        card,
 ...
 *   结界 → barrier
 */
export function cardFromBuilder(builderCard) {
    let cost;
    ...
    const rawAttack = builderCard.attack ?? builderCard.power;
    const rawHealth = builderCard.health ?? builderCard.life;
...
function drawCardsWithOverflow(state, player, count, opponentId) {
...
export function applyTurnStart(state, playerId, opponentId) {
...
function applyMulligan(player, cardIds) {
...
function resolveMulligans(state) {
...
export function submitMulligan(state, playerId, cardIds) {
...
function findFirstEmptyShikigamiSlot(player) {
    return player.shikigamiZone.findIndex((s) => s == null);
}
function canPlaceShikigami(player, toShikigamiSlot) {
...
function removeCardFromSource(state, player, from, cardId) {
    switch (from) {
        case "hand": {
...
function restoreCardToSource(player, from, card, spellEntry, shikigamiFromIndex, shikigamiEntry) {
    switch (from) {
        case "hand":
            player.hand.push(card);
            break;
        ...
        case "barrier":
            player.barrier = spellEntry ? { ...spellEntry } : newBarrierEntry(card);
            break;
        case "deck_top":
            player.deck.unshift(card);
            player.deckCount = player.deck.length;
...
export function moveCard(state, actorId, cardId, from, to, toShikigamiSlot, opponentId) {
...
export function toggleSpellReveal(state, actorId, cardId) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[actorId];
...
/** 公开/取消公开己方手牌给对手看 */
export function toggleHandReveal(state, actorId, cardId, reveal) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[actorId];
...
export function removeTokenCard(state, _actorId, cardId) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
...
export function placeShikigamiToken(state, _actorId, targetPlayerId, slotIndex, tokenKind) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
...
export function removeShikigamiToken(state, _actorId, targetPlayerId, slotIndex, tokenKind) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
    ...
    switch (tokenKind) {
        ...
        case "stun":
            ...
            break;
        case "silence":
            if (slot.silenceMarkers <= 0)
                return state;
            slot.silenceMarkers -= 1;
            break;
        case "poison":
            if (slot.poisonMarkers <= 0)
                return state;
            slot.poisonMarkers -= 1;
            break;
        case "weaken":
            if (slot.weakenMarkers <= 0)
                return state;
            slot.weakenMarkers -= 1;
            break;
        case "confusion":
            slot.confusionMarkers = slot.confusionMarkers ?? 0;
            if (slot.confusionMarkers <= 0)
                return state;
...
/** 在式神上添加/减少自定义标记 */
export function addCustomMarkerToShikigami(state, _actorId, targetPlayerId, slotIndex, markerName, delta) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
...
export function sanitizeMatchStateForPlayer(state, viewerId) {
    const stripSecrets = { ...state, mulliganChoices: undefined };
    const players = {};
...
export function adjustPlayerHp(state, actorId, delta) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[actorId];
...
/** 增减鬼火硬币 */
export function adjustGhostFire(state, actorId, delta) {
    if (state.winnerId)
        return state;
    const player = state.players[actorId];
    if (!player)
...
/** 增减鬼火数量 */
export function adjustFortuneFire(state, actorId, delta) {
    if (state.winnerId)
        return state;
    const player = state.players[actorId];
    if (!player)
...
/** 增减玩家的毒伤标记数量 */
export function adjustPlayerPoison(state, actorId, delta) {
    if (state.winnerId)
        return state;
    const player = state.players[actorId];
    if (!player)
...
/** 增减玩家的伤害记录 */
export function adjustPlayerDamage(state, actorId, delta) {
    if (state.winnerId)
        return state;
    const player = state.players[actorId];
    if (!player)
...
export function createMatchState(roomId, playerA, playerB) {
    // 随机决定先手（50/50）
...
export function playCard(state, actorId, cardId, targetPlayerId, zone) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const actor = state.players[actorId];
...
export function attack(state, actorId, attackerCardId, targetPlayerId, target, targetCardId) {
    if (state.winnerId || state.phase !== "playing" || state.currentPlayerId !== actorId) {
        return state;
    }
    const actor = state.players[actorId];
...
export function toggleSpellExhaust(state, actorId, cardId) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const actor = state.players[actorId];
...
export function toggleShikigamiExhaust(state, actorId, cardId) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const actor = state.players[actorId];
...
export function placeBarrierToken(state, _actorId, targetPlayerId, tokenKind) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
...
export function removeBarrierToken(state, _actorId, targetPlayerId, tokenKind) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
...
/** 添加/减少自定义标记到结界区 */
export function addCustomMarker(state, _actorId, targetPlayerId, markerName, delta) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
...
/** 添加/减少自定义标记到符咒区指定卡牌 */
export function addCustomMarkerToSpell(state, _actorId, targetPlayerId, cardId, markerName, delta) {
    if (state.winnerId || state.phase !== "playing")
        return state;
    const player = state.players[targetPlayerId];
    if (!player)
...
/** 添加/减少自定义标记到延伸区指定卡牌 */
export function addCustomMarkerToExtend(state, _actorId, targetPlayerId, cardId, markerName, delta) {
    if (state.winnerId || state.phase !== "playing")
        return state;
    const player = state.players[targetPlayerId];
    if (!player)
 ...
 * 敌方也可操作（用于查看）
 */
export function toggleExtendCardOrder(state, _actorId, targetPlayerId, slotIndex) {
    if (state.winnerId || state.phase !== "playing") {
        return state;
    }
    const player = state.players[targetPlayerId];
