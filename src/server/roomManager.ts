...
type Room = {
  ...
  spectators: Map<PlayerId, SpectatorEntry>;
...
export class RoomManager {
  ...
  /** 以观战者身份加入房间 */
  joinAsSpectator(roomId: RoomId, ws: WebSocket, playerId: PlayerId, name: string): MatchState | null {
    const room = this.rooms.get(roomId);
    if (!room) {
      throw new Error("room not found");
    }
  ...
  /** 向观战者广播完整对局状态（不脱敏） */
  broadcastToSpectators(roomId: RoomId, event: ServerEvent): void {
    const room = this.rooms.get(roomId);
    if (!room) return;
  ...
  /** 移除观战者 */
  removeSpectator(roomId: RoomId, playerId: PlayerId): void {
    const room = this.rooms.get(roomId);
    if (!room) return;
